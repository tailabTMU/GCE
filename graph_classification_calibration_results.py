...
def main():
    ...
    for dataset in ['enzymes', 'proteins']:
        dfs = {}
        final_sheet = []
        for model in models:
            # for calibration_method in ['uncalibrated', 'gcl', 'ece_contrib', 'brier_score']:
            for calibration_method in ['uncalibrated', 'gcl', 'brier_score']:
                results = []
                results_dir = pjoin(cwd, 'saved_info_graph_classification', f'{calibration_method}_{dataset}')
                    ...
                    for file_suffix in file_suffixes:
                        for split in range(1, num_folds + 1):
                            actual_file = pjoin(results_dir,
                                                f'{model}_split_{split}_test_true_classes{file_suffix}.txt')
                            pred_file = pjoin(results_dir,
                                              f'{model}_split_{split}_test_predicted_classes{file_suffix}.txt')
                            scores_file = pjoin(results_dir,
                                                f'{model}_split_{split}_test_logit_vals{file_suffix}.txt')
                            ...
                            if info != '':
                                ...
                                if info == 'dynamic_gamma':
                                    ...
                                    info = f'Gamma: {info}'
                            print(
                                f'Dataset: {dataset.upper()}, Model: {model_names[model]}, Calibration Method: {calibration_method.title().replace("_", "")}, Fold: {split}, Extra Info.: {info}')
                            print(
                                '====================================================================================')
                            print()
        ...
        with pd.ExcelWriter(pjoin(cwd, 'saved_info_graph_classification', f'results_{dataset}.xlsx')) as writer:
