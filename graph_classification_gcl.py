...
from utils.gnn import train, test, GraphCalibrationLoss
...
def main(args, gamma: float):
    ...
    if path.exists(pjoin(cwd, 'saved_info_graph_classification', f'gcl_{args.dataset}')) is False:
        os.mkdir(pjoin(cwd, 'saved_info_graph_classification', f'gcl_{args.dataset}'))
    ...
    for train_index, test_index in skf.split(indices, labels):
        ...
        ea = EarlyStopping(patience=10, verbose=False,
                           path=pjoin(cwd, 'saved_info_graph_classification', f'gcl_{args.dataset}',
                                      f'{model_name}_split_{split_number}_model_{gamma_txt}.pt'))
        root_ckpt_dir = pjoin(cwd, 'saved_info_graph_classification', f'gcl_{args.dataset}')
        ...
        with tqdm(range(params['epochs'])) as t:
            for epoch in t:
                ...
                torch.save(model.state_dict(), '{}.pkl'.format(pjoin(ckpt_dir, f"epoch_gamma_{gamma_txt}_{str(epoch)}")))
        ...
        with open(pjoin(cwd, 'saved_info_graph_classification', f'gcl_{args.dataset}',
                        f'{model_name}_split_{split_number}_test_logit_vals_{gamma_txt}.txt'),
                  'w+') as myfile:
            np.savetxt(myfile, logit_vals)

        with open(
                pjoin(cwd, 'saved_info_graph_classification', f'gcl_{args.dataset}',
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Training a Graph Classifier with GCL Loss')
