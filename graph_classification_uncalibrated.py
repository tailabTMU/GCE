...
def main(args):
    ...
    if path.exists(pjoin(cwd, 'saved_info_graph_classification', f'uncalibrated_{args.dataset}')) is False:
        os.mkdir(pjoin(cwd, 'saved_info_graph_classification', f'uncalibrated_{args.dataset}'))
    ...
    for train_index, test_index in skf.split(indices, labels):
        ...
        ea = EarlyStopping(patience=10, verbose=False,
                           path=pjoin(cwd, 'saved_info_graph_classification', f'uncalibrated_{args.dataset}',
                                      f'{model_name}_split_{split_number}_model.pt'))
        root_ckpt_dir = pjoin(cwd, 'saved_info_graph_classification', f'uncalibrated_{args.dataset}')
        ...
        with open(pjoin(cwd, 'saved_info_graph_classification', f'uncalibrated_{args.dataset}',
                        f'{model_name}_split_{split_number}_test_logit_vals.txt'),
                  'w+') as myfile:
            np.savetxt(myfile, logit_vals)

        with open(
                pjoin(cwd, 'saved_info_graph_classification', f'uncalibrated_{args.dataset}',
                      f'{model_name}_split_{split_number}_test_true_classes.txt'),
                'w+') as myfile:
            np.savetxt(myfile, true_classes)

        with open(pjoin(cwd, 'saved_info_graph_classification', f'uncalibrated_{args.dataset}',
                        f'{model_name}_split_{split_number}_test_predicted_classes.txt'),
                  'w+') as myfile:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Training a Graph Classifier with CrossEntropy Loss')
